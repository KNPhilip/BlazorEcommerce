...
namespace BlazorEcommerce.Server.Services.ProductService
{
    public sealed class ProductService : IProductService
    {
        ...
        {
            List<Product> products = await _context.Products
                .Where(p => p.Visible && !p.IsSoftDeleted && p.Category!.Url.ToLower().Equals(categoryUrl.ToLower()))
        ...
        public async Task<ResponseDto<ProductSearchResultDto>> SearchProductsAsync(string searchTerm, int page)
        {
            ...
            List<Product> products = await _context.Products
                .Where(p => p.Visible && !p.IsSoftDeleted && p.Title.ToLower()
                .Contains(searchTerm.ToLower()) || p.Visible && !p.IsSoftDeleted && p.Description.ToLower()
